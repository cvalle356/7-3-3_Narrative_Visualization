...
function render() {
  ...
  const path = $plot.selectAll('path')
    ...
    .attr('opacity', d => d.name == 'Total_Debt' || 'dti' ? 1 : 0.5)
  ...
  $plot.selectAll('.line-label')
    ...
    .attr('font-weight', d => d.name == 'Total_Debt' || 'dti' ? 700 : 400)
    .text(d => d.name)
    .attr('opacity', 0)
    .transition()
      .delay(4000)
...
d3.select('#opts')
  .on('change', function() {
    d3.selectAll(".line").remove();
	  ...
	  var newData = document.getElementById("opts").value.concat(".csv");
    var $plot = $svg1.append('g').attr('transform', `translate(${margin.left}, ${margin.top})`);

    //d3.csv(newData).then(console.log);
    d3.csv(newData).then(bindData);
});
